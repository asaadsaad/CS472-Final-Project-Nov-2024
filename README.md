...
The requirements below are for the standalone project. Please refer to the additional requirements section for groups. You will create a full-stack web application using React for the frontend and Express for the backend. The project will focus on state management, API interactions, and component-based architecture.
...
* For the protected actions (Add policy, Up-Vote, Down-Vote), you will need to implement a middleware at the server to verify the user registration state.
